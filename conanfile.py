...
class QtConan(ConanFile):
    ...
    options = dict({
        ...
        # "with_freetype": [True, False],  # Qt ignores FREETYPE_INCDIR
    ...
    def requirements(self):
        ...
        if self.options.with_pcre2:
            self.requires("pcre2/10.31@bincrafters/stable")
            self.options["pcre2"].build_pcre2_8 = False
            self.options["pcre2"].build_pcre2_16 = True
    ...
    def build(self):
        ...
        for package, var in libmap:
            if package in self.deps_cpp_info.deps:
                for include_path in self.deps_cpp_info[package].include_paths:
                    args.append("\"%s_INCDIR=%s\"" % (var, include_path))
                    break
